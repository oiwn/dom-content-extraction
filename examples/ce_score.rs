...
fn main_old() -> anyhow::Result<()> {
    ...
    for entry in fs::read_dir(gold_standard_dir)? {
        let entry = entry?;
        let path = entry.path();
        if path.extension().and_then(|s| s.to_str()) == Some("txt") {
            let file_name = path.file_stem().unwrap().to_str().unwrap();
            ...
            if html_path.exists() {
                if let Ok((precision, recall, f1)) =
                    process_file_pair(&path, &html_path)
                {
                    println!("File: {}", file_name);
                    println!("  Precision: {:.2}", precision);
                    println!("  Recall: {:.2}", recall);
    ...
    let avg_precision = total_precision / file_count as f64;
    let avg_recall = total_recall / file_count as f64;
    let avg_f1 = total_f1 / file_count as f64;
    ...
    println!("Clean content length: {}", clean_content.len());
    println!("Extracted content length: {}", extracted_content.len());
    println!("LSC length: {}", lcs_length);
    println!("Precision: {:.2}", precision);
    println!("Recall: {:.2}", recall);
